...
/*
    const updateDocument = async (id) => {
        try{
            ...
            console.log(result); // ye kewal batayega ki kitne document modify huaa hai
        }catch(err){
            console.log(err);
        }
...
const updateDocument = async (id) => {
    try{
        const result = await Playlist.findByIdAndUpdate({_id:id},{
            $set : {
                name : "Javascript thapa Technical"
            }
            },{
                
                useFindAndModify : false
            });
        console.log(result); // ye kewal batayega ki kitne document modify huaa hai, es prakar se update kiya huaa data nahi ata hai update karne se pahle jo data tha o aata hai lekin data updata ho jata hai
    }catch(err){
        console.log(err);
    }
...
const updateDocument = async (id) => {
    try{
        ...
        console.log(result); // ye kewal batayega ki kitne document modify huaa hai, es prakar karne updated data aata hai
    }catch(err){
        console.log(err);
    }
    ...
    updateDocument("64aaea2f118a2684b6fd7e01");
